...
bool RH_RF95::init() {
  ...
  if (spiRead(RH_RF95_REG_01_OP_MODE) !=
      (RH_RF95_MODE_SLEEP | RH_RF95_LONG_RANGE_MODE)) {
  ...
  if (_interruptPin != RH_INVALID_PIN) {
    ...
    // actually the interrupt number. You have to figure out the
    // interruptnumber-to-interruptpin mapping yourself based on knwledge of
    // what Arduino board you are running on.
...
void RH_RF95::handleInterrupt() {
  ...
  // we need the RF95 IRQ to be level triggered, or we ……have slim chance of
  // missing events
  ...
  // (kevinh - I think the root cause we want level triggered interrupts here -
  // not edge.  Because edge allows us to miss handling secondard interrupts
  // that occurred while this ISR was running.  Better to instead, configure the
  // interrupts as level triggered and clear pending at the _beginning_ of the
  // ISR.  If any interrupts occur while handling the ISR, the signal will
  // remain asserted and our ISR will be reinvoked to handle that case) kevinh:
  // turn this off until root cause is known, because it can cause missed
  // interrupts! spiWrite(RH_RF95_REG_12_IRQ_FLAGS, 0xff); // Clear all IRQ
  // flags
  ...
  //    if (_mode == RHModeRx && irq_flags & (RH_RF95_RX_TIMEOUT |
...
bool RH_RF95::send(const uint8_t *data, uint8_t len) {
  ...
  // when Tx is done, interruptHandler will fire and radio mode will return to
  // STANDBY
  return true;
}
...
bool RH_RF95::printRegisters() {
  ...
  uint8_t registers[] = {0x01, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,  0x0c, 0x0d,
                         0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x014, 0x15, 0x16,
                         0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d,  0x1e, 0x1f,
                         0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26,  0x27, 0x4b};
...
void RH_RF95::setTxPower(int8_t power, bool useRFO) {
  ...
  // Sigh, different behaviours depending on whether the module use PA_BOOST or
  ...
  if (useRFO) {
    ...
    // power levels. We will use it for 8, 19 and 20dBm
    if (power > 17) {
    ...
    // (+3dBm if DAC enabled)
    // fix for dramatically decreased range in imparrison to RadioLib
    spiWrite(RH_RF95_REG_09_PA_CONFIG,
             RH_RF95_PA_SELECT | RH_RF95_MAX_POWER | (power - 2));
  }
}
...
void RH_RF95::enableTCXO(bool on) {
  if (on) {
    while ((spiRead(RH_RF95_REG_4B_TCXO) & RH_RF95_TCXO_TCXO_INPUT_ON) !=
      ...
      sleep();
      spiWrite(RH_RF95_REG_4B_TCXO,
               (spiRead(RH_RF95_REG_4B_TCXO) | RH_RF95_TCXO_TCXO_INPUT_ON));
    }
  } else {
    while ((spiRead(RH_RF95_REG_4B_TCXO) & RH_RF95_TCXO_TCXO_INPUT_ON)) {
      sleep();
...
int RH_RF95::lastSNR() { return _lastSNR; }
...
void RH_RF95::setSpreadingFactor(uint8_t sf) {
  ...
  spiWrite(RH_RF95_REG_1E_MODEM_CONFIG2,
           (spiRead(RH_RF95_REG_1E_MODEM_CONFIG2) & ~RH_RF95_SPREADING_FACTOR) |
               sf);
...
void RH_RF95::setSignalBandwidth(long sbw) {
  ...
  spiWrite(RH_RF95_REG_1D_MODEM_CONFIG1,
           (spiRead(RH_RF95_REG_1D_MODEM_CONFIG1) & ~RH_RF95_BW) | bw);
  // check if low data rate bit should be set or cleared
  setLowDatarate();
}
...
void RH_RF95::setCodingRate4(uint8_t denominator) {
  ...
  spiWrite(RH_RF95_REG_1D_MODEM_CONFIG1,
           (spiRead(RH_RF95_REG_1D_MODEM_CONFIG1) & ~RH_RF95_CODING_RATE) | cr);
}
...
void RH_RF95::setLowDatarate() {
  ...
  // "To avoid issues surrounding  drift  of  the  crystal  reference oscillator
  // due  to  either  temperature  change or  motion,the  low  data  rate
  // optimization  bit  is  used. Specifically for 125  kHz  bandwidth  and  SF
  // =  11  and  12, this  adds  a  small  overhead  to increase robustness to
  // reference frequency variations over the timescale of the LoRa packet."
  ...
  float bw_tab[] = {7800,  10400, 15600,  20800,  31250,
  ...
  uint8_t current = spiRead(RH_RF95_REG_26_MODEM_CONFIG3) &
                    ~RH_RF95_LOW_DATA_RATE_OPTIMIZE; // mask off the LDR bit
  if (symbolTime > 16.0)
    spiWrite(RH_RF95_REG_26_MODEM_CONFIG3,
...
void RH_RF95::setPayloadCRC(bool on) {
  ...
  uint8_t current = spiRead(RH_RF95_REG_1E_MODEM_CONFIG2) &
                    ~RH_RF95_PAYLOAD_CRC_ON; // mask off the CRC
